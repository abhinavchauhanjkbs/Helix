...
const Products = () => {
  ...
  const pageCount = Math.max(1, Math.ceil(productData.length / pageSize));
  const isFirst = currentPage === 0;
  const isLast = currentPage === pageCount - 1;
  const pages = Array.from({ length: pageCount }, (_, pageIndex) =>
    productData.slice(pageIndex * pageSize, pageIndex * pageSize + pageSize)
  );
    ...
    <div className="max-w-7xl container flex flex-col bg-white px-4 sm:px-5 py-8 md:py-10 lg:p-16 w-full space-y-6 md:space-y-8 items-center justify-center mx-auto">
      ...
      <div className="flex flex-col items-center justify-center space-y-3 md:space-y-4">
        <div className="flex items-center justify-center gap-2">
          <h1 className="italic font-bold text-sm sm:text-base">Smart Product Solutions</h1>
          <img src="/assets/line.png" alt="Line Icon" className="w-16 sm:w-20 h-1" loading="lazy" decoding="async" />
        </div>
        <h1 className="text-2xl sm:text-3xl md:text-[40px] lg:text-[46px] text-center font-sans font-normal">
          Looking for Appropriate{" "}
          <span className="font-bold text-primary">IT Solutions?</span>
        </h1>
        <p className="text-gray-500 max-w-xl text-center text-sm sm:text-base md:text-lg p-2">
          Empowering businesses with customized IT solutions that enhance
          efficiency, security, and growth through innovative technology.
        </p>
      </div>
      ...
      <div className="relative w-full max-w-7xl mx-auto">
        ...
        <button
          className={`absolute top-1/2 -left-2 sm:-left-1 md:-left-4 transform -translate-y-1/2 w-8 h-8 sm:w-12 sm:h-12 md:w-12 md:h-12 rounded-full z-20 transition-all duration-200 flex items-center justify-center outline-none border-none ${
            isFirst
        ...
        <button
          className={`absolute top-1/2 -right-2 sm:-right-1 md:-right-4 transform -translate-y-1/2 w-8 h-8 sm:w-12 sm:h-12 md:w-12 md:h-12 rounded-full z-20 transition-all duration-200 flex items-center justify-center outline-none border-none ${
            isLast
              ? "bg-gray-200 text-gray-400 cursor-not-allowed"
              : "bg-[#842326] text-white hover:bg-[#6a1c1e] shadow-lg hover:scale-110"
          }`}
      ...
      {/* Page indicators (one dot per page of 4 cards) */}
